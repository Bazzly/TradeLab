...
| Economic Calendar | **Financial Modeling Prep** (free-tier API key) | Free key, calendar endpoint confirmed to exist via direct request. Trading Economics' old no-signup "guest" access is dead — confirmed discontinued as of 2026-08-19, don't build against it |
...
**Resolved (2026-08-19) — providers picked for the last two Core Engines, both built (not yet live-verified, no key supplied yet):**
- Economic Calendar: **Financial Modeling Prep**, `lib/economic_calendar/fmp.py` + Calendar page. Chosen over Trading Economics after confirming directly (via a live request) that TE's old no-signup "guest" calendar access has been discontinued — don't try that path again. FMP and Finnhub both confirmed to have a real free-tier calendar endpoint gated behind a free API key; FMP picked as primary.
- Trade Review AI: **Google Gemini** (`gemini-2.5-flash`), `lib/engines/trade_review.py`, wired into the Journal page as an "AI Trade Review" section under closed trades. Confirmed Gemini has a genuinely free, ongoing tier (not trial credits) before picking it over Claude/OpenAI, which don't. The review prompt is built to grade process against what's actually logged on the JournalEntry, not P&L — and explicitly told not to claim knowledge of a linked strategy/signal object, since strategy persistence (Section 4.5) isn't built yet.
- Both degrade gracefully without their API key (same pattern as Neon/OANDA) and pass headless regression; validation logic (e.g. "can't review an OPEN trade") verified directly. Full live verification (a real Gemini review, a real FMP calendar pull) is still pending — needs `FMP_API_KEY` / `GEMINI_API_KEY` supplied.
